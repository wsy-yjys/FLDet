...
class BaseTrainer:
    ...
    def _do_train(self, world_size=1):
        ...
        if self.args.decay_aug:
            print("Enable decay data aug training strategy")
        for epoch in range(self.start_epoch, self.epochs):
            ...
            if self.args.decay_aug:
                # -----------Decay aug----------- #
                if self.args.close_mixup and epoch == (self.epochs - self.args.close_mixup):
                    LOGGER.info('Closing dataloader mixup')
                    self.train_loader.dataset.close_mixup_only(hyp=self.args)
                    self.train_loader.reset()
                if self.args.close_copy_paste and epoch == (self.epochs - self.args.close_copy_paste):
                    LOGGER.info('Closing dataloader copy_paste')
                    self.train_loader.dataset.close_copy_paste_only(hyp=self.args)
                    self.train_loader.reset()
                if self.args.close_mosaic and epoch == (self.epochs - self.args.close_mosaic):
                    LOGGER.info('Closing dataloader mosaic')
                    self.train_loader.dataset.close_mosaic_only(hyp=self.args)
                    self.train_loader.reset()
